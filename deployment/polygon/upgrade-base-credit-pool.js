...
async function upgradeHDT() {
    const upgraded = await getUpgradedContract("BaseCreditHDT");
    if (upgraded) {
        console.log("BaseCreditHDT is upgraded already!");
        return;
    }

    if (!deployedContracts["BaseCreditHDTImpl"]) {
        throw new Error("BaseCreditHDTImpl not deployed yet!");
    }
