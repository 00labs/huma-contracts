...
async function deployContracts() {
    ...
    const baseCreditPoolTL = await deploy("TimelockController", "BSOSPoolTimelock", [
        0,
        [POOL_OWNER_SAFE],
        [deployer.address],
    ]);

    const baseCreditPoolProxyAdminTL = await deploy("TimelockController", "BSOSPoolProxyAdminTimelock", [
        0,
        [POOL_OWNER_SAFE],
        [deployer.address],
    ]);

    const bc_feeManager = await deploy("BaseFeeManager", "BSOSPoolFeeManager");
    const bc_hdtImpl = await deploy("HDT", "ArfNewHDTImpl");
    const bc_hdt = await deploy("TransparentUpgradeableProxy", "BSOSHDT", [
        bc_hdtImpl.address,
        baseCreditPoolProxyAdminTL.address,
        [],
    ]);
    const bc_poolConfig = await deploy("BasePoolConfig", "BSOSPoolConfig");

    const bc_poolImpl = await deploy("BaseCreditPool", "ArfNewPoolImpl");
    const bc_pool = await deploy("TransparentUpgradeableProxy", "BSOSPool", [
        bc_poolImpl.address,
        baseCreditPoolProxyAdminTL.address,
        [],
    ]);
