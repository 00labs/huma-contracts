...
async function initBaseCreditPoolHumaConfig() {
    const initilized = await getInitilizedContract("BaseCreditPoolHumaConfig");
    if (initilized) {
        console.log("BaseCreditPoolHumaConfig is already initialized!");
        return;
    }

    if (!deployedContracts["BaseCreditPoolHumaConfig"]) {
        throw new Error("BaseCreditPoolHumaConfig not deployed yet!");
    }

    if (!deployedContracts["BaseCreditPoolHumaConfigTimelock"]) {
        throw new Error("BaseCreditPoolHumaConfigTimelock not deployed yet!");
    }
    ...
    const HumaConfig = await hre.ethers.getContractFactory("HumaConfig");
    const humaConfig = HumaConfig.attach(deployedContracts["BaseCreditPoolHumaConfig"]);

    const TimelockController = await hre.ethers.getContractFactory("TimelockController");
    const humaConfigTL = TimelockController.attach(deployedContracts["BaseCreditPoolHumaConfigTimelock"]);

    await sendTransaction("HumaConfig", humaConfig, "setProtocolDefaultGracePeriod", [
        30 * 24 * 3600,
    ]);
    await sendTransaction("HumaConfig", humaConfig, "setTreasuryFee", [1000]);
    await sendTransaction("HumaConfig", humaConfig, "setEANFTContractAddress", [
        deployedContracts["EANFT"],
    ]);
    ...
    await updateInitilizedContract("BaseCreditPoolHumaConfig");
}
...
async function initBaseCreditPoolConfig() {
    ...
    const cap = BN.from(1_000_000).mul(BN.from(10).pow(BN.from(decimals)));
    console.log("cap: " + cap);
    await sendTransaction("BaseCreditPoolConfig", poolConfig, "setPoolLiquidityCap", [
        cap,
    ]);
    ...
    const maxCL = BN.from(1_000).mul(BN.from(10).pow(BN.from(decimals)));
    console.log("maxCL: " + maxCL);
    await sendTransaction("BaseCreditPoolConfig", poolConfig, "setMaxCreditLine", [
        maxCL,
    ]);
...
async function prepareBaseCreditPool() {
    ...
    await sendTransaction("BaseCreditPool", pool, "addApprovedLender", [
        deployer.address,
    ]);
