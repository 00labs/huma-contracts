...
describe("Base Credit Pool", function () {
    ...
    describe("BaseCreditPool settings", function () {
        it("Should not allow credit line to be changed when protocol is paused", async function () {
            ...
            await expect(
                poolContract
                    ...
                    .changeCreditLine(borrower.address, toTKN(1000000))
        ...
        it("Should not allow non-EA to change credit line", async function () {
            await expect(
                poolContract.connect(borrower).changeCreditLine(borrower.address, toTKN(1000000))
        ...
        it("Should not allow credit line to be changed to above maximal credit line", async function () {
            await expect(
                poolContract
                    .connect(eaServiceAccount)
                    .changeCreditLine(borrower.address, toTKN(50000000))
        ...
        it("Should allow credit limit to be changed", async function () {
            ...
            expect(result.creditLimit).to.equal(toTKN(1000000));
        });
        ...
        it("Should not delete a credit line when there is balance due when set credit limit to 0", async function () {
            ...
            await poolContract.connect(borrower).requestCredit(toTKN(4000), 30, 12);
            await poolContract
                .connect(eaServiceAccount)
                .approveCredit(borrower.address, toTKN(4000), 30, 12, 1217);
            ...
            await testTokenContract.mint(borrower.address, toTKN(1080));
            await testTokenContract.connect(borrower).approve(poolContract.address, toTKN(4040));
            await poolContract.connect(borrower).makePayment(borrower.address, toTKN(4040));
    ...
    describe("HDT", async function () {
        ...
        it("Should reject non-owner to call mintAmount()", async function () {
            await expect(
                hdtContract.connect(lender).mintAmount(lender.address, toTKN(1_000_000))
            ).to.be.revertedWith("notPool");
        });
        it("Should reject non-owner to call burnAmount()", async function () {
            await expect(
                hdtContract.connect(lender).burnAmount(lender.address, toTKN(1_000_000))
            ).to.be.revertedWith("notPool");
        });
    });
    ...
    describe("Borrowing request", function () {
        ...
        it("Should reject loan requests while zero period", async function () {
            ...
            await expect(
                poolContract.connect(borrower).requestCredit(toTKN(1_000_000), 30, 0)
            ).to.be.revertedWith("requestedCreditWithZeroDuration()");
        });
        it("Should reject loan requests while protocol is paused", async function () {
            await humaConfigContract.connect(poolOwner).pause();
            await expect(
                poolContract.connect(borrower).requestCredit(toTKN(1_000_000), 30, 12)
            ).to.be.revertedWith("protocolIsPaused()");
        });
        ...
        it("Shall reject request loan greater than limit", async function () {
            await expect(
                poolContract.connect(borrower).requestCredit(toTKN(10_000_001), 30, 12)
            ).to.be.revertedWith("greaterThanMaxCreditLine()");
        });
        ...
        it("Shall allow loan request", async function () {
            ...
            expect(loanInformation.creditLimit).to.equal(toTKN(1_000_000));
        ...
        it("Shall allow new request if there is existing loan in Requested state", async function () {
            await poolContract.connect(borrower).requestCredit(toTKN(1_000), 30, 12);
            await poolContract.connect(borrower).requestCredit(toTKN(2_000), 60, 24);
            const loanInformation = await getCreditInfo(poolContract, borrower.address);
            expect(loanInformation.creditLimit).to.equal(toTKN(2_000));
        ...
        it("Shall reject loan requests if there is an outstanding loan with outstanding balance", async function () {
            await poolContract.connect(borrower).requestCredit(toTKN(3_000), 30, 12);
            await poolContract
                .connect(eaServiceAccount)
                .approveCredit(borrower.address, toTKN(3000), 30, 12, 1217);
            await poolContract.connect(borrower).drawdown(toTKN(2_000));

            await expect(
                poolContract.connect(borrower).requestCredit(toTKN(1_000), 30, 12)
        ...
        it("Shall allow new request if existing loan has been paid off", async function () {
            await poolContract.connect(borrower).requestCredit(toTKN(3_000), 30, 12);
            await expect(
                poolContract.connect(borrower).makePayment(borrower.address, toTKN(3_000))
            ...
            await poolContract.connect(borrower).drawdown(toTKN(3_000));
            await testTokenContract.connect(borrower).mint(borrower.address, toTKN(2_000));
            await testTokenContract.connect(borrower).approve(poolContract.address, toTKN(3_100));
            await poolContract.connect(borrower).makePayment(borrower.address, toTKN(3_100));
            await poolContract.connect(borrower).requestCredit(toTKN(4_000), 90, 36);
            const loanInformation = await getCreditInfo(poolContract, borrower.address);
            expect(loanInformation.creditLimit).to.equal(toTKN(4_000));
    ...
    describe("Drawdown", function () {
        ...
        it("Should not allow loan funding while protocol is paused", async function () {
            await humaConfigContract.connect(poolOwner).pause();
            await expect(poolContract.connect(borrower).drawdown(toTKN(400))).to.be.revertedWith(
                "protocolIsPaused()"
            );
        });
        ...
        it("Should reject drawdown if the combined balance is higher than the credit limit", async function () {
            ...
            await expect(poolContract.connect(borrower).drawdown(toTKN(4000))).to.be.revertedWith(
                "creditLineExceeded()"
            );
            await testTokenContract.mint(borrower.address, toTKN(11000));
            await testTokenContract
                .connect(borrower)
                .approve(poolContract.address, toTKN(1_000_000));
            await poolContract.connect(borrower).makePayment(borrower.address, toTKN(1_000_000));
        });
        ...
        it("Should reject if the borrowing amount is less than platform fees", async function () {
            await poolContract
                .connect(eaServiceAccount)
                .approveCredit(borrower.address, toTKN(3_000), 30, 12, 1217);
            await expect(poolContract.connect(borrower).drawdown(toTKN(100))).to.be.revertedWith(
        ...
        it("Should reject if the borrowing amount is more than approved", async function () {
            await poolContract
                ...
                .approveCredit(borrower.address, toTKN(1_000_000), 30, 12, 1217);
            await expect(
                poolContract.connect(borrower).drawdown(toTKN(1_100_000))
        ...
        it("Borrow less than approved amount", async function () {
            ...
            await expect(poolContract.connect(borrower).drawdown(toTKN(100_000)))
                .to.emit(poolContract, "DrawdownMade")
                .withArgs(borrower.address, toTKN(100_000), toTKN(98_000));
            ...
            await testTokenContract.mint(borrower.address, toTKN(2000));

            // Please note since the credit is paid back instantly, no interest is actually charged.
            await testTokenContract.connect(borrower).approve(poolContract.address, toTKN(100000));
            await poolContract.connect(borrower).makePayment(borrower.address, toTKN(100000));
        });
        ...
        it("Should reject drawdown in the final pay period of the credit line", async function () {
            await poolContract
                ...
                .approveCredit(borrower.address, toTKN(1_000_000), 30, 12, 1217);
            await poolContract.connect(borrower).drawdown(toTKN(1_000_000));
            await testTokenContract.mint(borrower.address, toTKN(21002));
            await testTokenContract
                .connect(borrower)
                .approve(poolContract.address, toTKN(1_010_002));
            await poolContract.connect(borrower).makePayment(borrower.address, toTKN(1_010_002));
        ...
        it("Should reject drawdown when account is late in payments", async function () {
            await poolContract
                .connect(eaServiceAccount)
                .approveCredit(borrower.address, toTKN(1_00_000), 30, 12, 1217);
            await poolContract.connect(borrower).drawdown(toTKN(100_000));
    ...
    describe("Credit expiration without a timely first drawdown", function () {
        ...
        it("Can borrow if no credit expiration has been setup for the pool", async function () {
            ...
            await expect(poolContract.connect(borrower).drawdown(toTKN(1_000_000)));
            let creditInfo = await poolContract.creditRecordMapping(borrower.address);
        ...
        it("Expiration window does not apply after initial drawdown", async function () {
            ...
            await poolContract
                .connect(eaServiceAccount)
                .approveCredit(borrower.address, toTKN(1_000_000), 30, 12, 1217);
            await expect(poolContract.connect(borrower).drawdown(toTKN(500_000)));
            let creditInfo = await poolContract.creditRecordMapping(borrower.address);
            expect(creditInfo.unbilledPrincipal).to.equal(toTKN(500_000));

            await advanceClock(6);

            await poolContract.connect(borrower).drawdown(toTKN(500_000));
            creditInfo = await poolContract.creditRecordMapping(borrower.address);
            expect(creditInfo.unbilledPrincipal).to.equal(toTKN(1_000_000));
        });
    });
    ...
    describe("Payback", function () {
        ...
        it("Should reject if payback amount is zero", async function () {
            await testTokenContract.connect(borrower).approve(poolContract.address, toTKN(1000));
            await expect(
        ...
        it("Process payback", async function () {
            ...
            await testTokenContract.connect(borrower).approve(poolContract.address, toTKN(11002));

            await poolContract.connect(borrower).makePayment(borrower.address, toTKN(11002));
        ...
        it("Automatic payback", async function () {
            ...
            await poolContract
                ...
                .makePayment(borrower.address, toTKN(11002));
    ...
    describe("Quick payback", function () {
        it("Process payback", async function () {
            ...
            await testTokenContract.mint(borrower.address, toTKN(1_000_100));

            let oldBalance = await testTokenContract.balanceOf(borrower.address);
            await testTokenContract
                .connect(borrower)
                .approve(poolContract.address, toTKN(1_000_500));

            await expect(
                poolContract.connect(borrower).makePayment(borrower.address, toTKN(1_000_500))
            ).to.emit(poolContract, "PaymentMade");
            let newBalance = await testTokenContract.balanceOf(borrower.address);
            expect(oldBalance.sub(newBalance)).to.be.within(toTKN(1_000_000), toTKN(1_000_500));
            ...
            checkRecord(r, rs, toTKN(1_000_000), 0, dueDate, 0, 0, 0, 0, 11, 1217, 30, 3, 0);
        });
    });
    ...
    describe("Multiple immediate payback towards payoff", function () {
        it("Multiple immediate payback", async function () {
            ...
            await testTokenContract
                .connect(borrower)
                .approve(poolContract.address, toTKN(900_000));

            await expect(
                poolContract.connect(borrower).makePayment(borrower.address, toTKN(900_000))
            ).to.emit(poolContract, "PaymentMade");
            let newBalance = await testTokenContract.balanceOf(borrower.address);
            ...
            await testTokenContract
                .connect(borrower)
                .approve(poolContract.address, toTKN(120_000));

            await expect(
                poolContract.connect(borrower).makePayment(borrower.address, toTKN(120_000))
            ).to.emit(poolContract, "PaymentMade");
            ...
            checkRecord(r, rs, toTKN(1_000_000), 0, dueDate, 0, 0, 0, 0, 10, 1217, 30, 3, 0);
        });
    });
    ...
    describe("Multiple borrowing to form positive correction in payoff", function () {
        it("Multiple consecutive borrowing", async function () {
            ...
            await testTokenContract
                .connect(borrower)
                .approve(poolContract.address, toTKN(1_100_000));
            await expect(
                poolContract.connect(borrower).makePayment(borrower.address, toTKN(1_100_000))
            ).to.emit(poolContract, "PaymentMade");
            r = await poolContract.creditRecordMapping(borrower.address);
            rs = await poolContract.creditRecordStaticMapping(borrower.address);
            checkRecord(r, rs, toTKN(1_000_000), 0, dueDate, 0, 0, 0, 0, 11, 1217, 30, 3, 0);
    ...
    describe("makePayment after account deleted", function () {
        it("Shall revert makePayment() if the account has been deleted", async function () {
            await poolConfigContract.connect(poolOwner).setAPR(1217);
            await poolContract.connect(borrower).requestCredit(toTKN(1_000_000), 30, 2);

            await poolContract
                .connect(eaServiceAccount)
                .approveCredit(borrower.address, toTKN(1_000_000), 30, 2, 1217);

            await poolContract.connect(borrower).drawdown(toTKN(500_000));

            await testTokenContract.mint(borrower.address, toTKN(1_000_000));
            await testTokenContract
                .connect(borrower)
                .approve(poolContract.address, toTKN(1_000_000));
            await poolContract.connect(borrower).makePayment(borrower.address, toTKN(100_000));

            await advanceClock(30);
            await expect(
                poolContract.connect(borrower).makePayment(borrower.address, toTKN(500_000))
            ).to.emit(poolContract, "PaymentMade");
            r = await poolContract.creditRecordMapping(borrower.address);
            rs = await poolContract.creditRecordStaticMapping(borrower.address);
            checkRecord(r, rs, toTKN(1_000_000), 0, "SKIP", 0, 0, 0, 0, 0, 1217, 30, 0, 0);

            // Additional payment after payoff
            await expect(
                poolContract.connect(borrower).makePayment(borrower.address, toTKN(1000))
            ).to.be.revertedWith("creditLineNotInStateForMakingPayment");
        });
    });
    ...
    describe("Quick large amount payback (for getDueInfo overflow)", function () {
        it("Quick follow-up borrowing", async function () {
            ...
            await poolConfigContract.connect(poolOwner).setAPR(1217);
            await poolContract.connect(borrower).requestCredit(toTKN(1_500_000), 30, 12);

            await poolContract
                .connect(eaServiceAccount)
                .approveCredit(borrower.address, toTKN(1_500_000), 30, 12, 1217);
            ...
            await expect(
                ...
                .to.emit(poolContract, "PaymentMade")
                .withArgs(borrower.address, toTKN(900_000), 0, 110002739726, borrower.address);
            ...
            await advanceClock(90);
            dueDate += 3 * 2592000;
            await testTokenContract.connect(borrower).mint(borrower.address, toTKN(50_000));
            await testTokenContract.connect(borrower).approve(poolContract.address, toTKN(50_000));
            await expect(
                poolContract.connect(borrower).makePayment(borrower.address, toTKN(50_000))
            ).to.emit(poolContract, "PaymentMade");
            r = await poolContract.creditRecordMapping(borrower.address);
            rs = await poolContract.creditRecordStaticMapping(borrower.address);
            await checkRecord(
            ...
            await testTokenContract.connect(borrower).mint(borrower.address, toTKN(60_000));
            await testTokenContract.connect(borrower).approve(poolContract.address, toTKN(60_000));
            await expect(
                poolContract.connect(borrower).makePayment(borrower.address, toTKN(60_000))
            )
                .to.emit(poolContract, "PaymentMade")
                .withArgs(borrower.address, 59215672089, 0, 0, borrower.address);
            r = await poolContract.creditRecordMapping(borrower.address);
            rs = await poolContract.creditRecordStaticMapping(borrower.address);
            checkRecord(r, rs, toTKN(1_500_000), 0, dueDate, 0, 0, 0, 0, 8, 1217, 30, 3, 0);
        });
    });
    ...
    describe("Default", function () {
        ...
        it("Default flow", async function () {
            ...
            expect(await hdtContract.totalSupply()).to.equal(toTKN(5_000_000));
            ...
            expect(await hdtContract.withdrawableFundsOf(lender.address)).to.equal(1593863599866);
        ...
        it("Post-default payment", async function () {
            ...
            await testTokenContract.connect(borrower).mint(borrower.address, toTKN(200_000));
            await poolConfigContract.connect(poolOwner).setPoolDefaultGracePeriod(60);
            ...
            await testTokenContract.connect(borrower).approve(poolContract.address, toTKN(25_000));

            await poolContract.connect(borrower).makePayment(borrower.address, toTKN(25_000));
            ...
            expect(await hdtContract.withdrawableFundsOf(lender.address)).to.equal(1603863599866);
            ...
            await testTokenContract
                .connect(borrower)
                .approve(poolContract.address, toTKN(1_050_000));

            await expect(
                poolContract.connect(borrower).makePayment(borrower.address, toTKN(1_050_000))
            )
                .to.emit(poolContract, "PaymentMade")
                .withArgs(borrower.address, toTKN(1_050_000), 0, 26514442977, borrower.address);
            ...
            expect(await hdtContract.withdrawableFundsOf(lender.address)).to.equal(2020640000002);
            ...
            await testTokenContract.connect(borrower).approve(poolContract.address, toTKN(27_000));

            await expect(
                poolContract.connect(borrower).makePayment(borrower.address, toTKN(27_000))
            )
                .to.emit(poolContract, "PaymentMade")
                .withArgs(borrower.address, 22915526737, 0, 0, borrower.address);
            ...
            expect(await poolContract.totalPoolValue()).to.equal(5065349316050);
            expect(await hdtContract.withdrawableFundsOf(poolOwnerTreasury.address)).to.equal(
                1013069863210
            );
            expect(await hdtContract.withdrawableFundsOf(evaluationAgent.address)).to.equal(
            ...
            expect(await hdtContract.withdrawableFundsOf(lender.address)).to.equal(2026139726420);

            accruedIncome = await poolConfigContract.accruedIncome();
            expect(accruedIncome.protocolIncome).to.equal(21783105346);
            expect(accruedIncome.poolOwnerIncome).to.equal(5445776334);
            expect(accruedIncome.eaIncome).to.equal(16337329007);
    ...
    describe("Protocol/Pool Owner/EA fee", function () {
        it("Should not allow non-protocol-owner to withdraw protocol", async function () {
            await expect(poolConfigContract.withdrawProtocolFee(toTKN(1))).to.be.revertedWith(
                "notProtocolOwner"
            );
        });

        it("Should not allow non-pool-owner to withdraw pool owner fee", async function () {
            await expect(poolConfigContract.withdrawPoolOwnerFee(toTKN(1))).to.be.revertedWith(
                "notPoolOwnerTreasury"
            );
        });

        it("Should not allow non-poolOwner or EA withdraw EA fee", async function () {
            await expect(poolConfigContract.withdrawEAFee(toTKN(1))).to.be.revertedWith(
                "notPoolOwnerOrEA"
            );
        });
        ...
        it("Should not withdraw protocol fee while amount > withdrawable", async function () {
            ...
            await expect(
                poolConfigFromProtocolOwner.withdrawProtocolFee(toTKN(1))
        ...
        it("Should not withdraw pool owner fee if amount > withdrawable", async function () {
            ...
            await expect(
                poolConfigFromPoolOwner.withdrawPoolOwnerFee(toTKN(1))
            ).to.be.revertedWith("withdrawnAmountHigherThanBalance");
        });
        ...
        it("Should not withdraw ea fee while amount > withdrawable", async function () {
            ...
            await expect(poolConfigFromPoolOwner.withdrawEAFee(toTKN(1))).to.be.revertedWith(
