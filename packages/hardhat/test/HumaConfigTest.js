...
describe("Huma Config", function () {
    ...
    describe("Initial Value", function () {
        ...
        it("Should have the right initial default grace period", async function () {
            expect(await configContract.getDefaultGracePeriod()).to.equal(
                5 * 3600 * 24
            );
        });
    ...
    describe("Change Default Grace Period", function () {
        it("Should disallow non-proto-admin to change default grace period", async function () {
            await expect(
                configContract
                    .connect(origAdmin)
                    .setDefaultGracePeriod(10 * 24 * 3600)
            ).to.be.revertedWith("HumaConfig:PROTO_ADMIN_REQUIRED");
            await expect(
                configContract
                    .connect(treasury)
        ...
        it("Should disallow default grace period to be shorten than one day", async function () {
            await expect(
                configContract
                    .connect(newAdmin)
                    .setDefaultGracePeriod(12 * 3600)
            ).to.be.revertedWith("HumaConfig:GRACE_PERIOD_TOO_SHORT");
            await expect(
                configContract.connect(newAdmin).setDefaultGracePeriod(0)
            ).to.be.revertedWith("HumaConfig:GRACE_PERIOD_TOO_SHORT");
        });
        ...
        it("Should be able to reset default grace period to be longer than 1 day", async function () {
            await expect(
                ...
                .to.emit(configContract, "DefaultGracePeriodChanged")
                .withArgs(10 * 24 * 3600);
            expect(await configContract.getDefaultGracePeriod()).to.equal(
                10 * 24 * 3600
            );
